...
import { X, QrCode, ChevronDown, ChevronRight, Search } from 'lucide-react';
import { useAuth } from '@/contexts/auth-context';
import { ALL_NAV_HREFS, NAV_SECTIONS, isNavItemVisible, isNavSectionVisible } from '@/lib/navigation';
...
function isActiveRoute(pathname: string, href: string, activeHref: string | null): boolean {
  const cleanHref = href.split('#')[0];
  return activeHref === cleanHref;
}
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const activeHref = useMemo(() => resolveActiveHref(pathname, ALL_NAV_HREFS), [pathname]);

  const [collapsedSections, setCollapsedSections] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(
      NAV_SECTIONS.filter((s) => s.isCollapsible).map((s) => [s.category, true])
    )
  );
      ...
      if (section.isCollapsible) {
        const hasActiveChild = section.items.some(
          (item) =>
            ...
            isActiveRoute(pathname, item.href, activeHref)
        );
        if (hasActiveChild) {
  ...
  const filteredSections = useMemo(() => {
    const base = NAV_SECTIONS.filter((section) => isNavSectionVisible(section, user)).map((section) => ({
      ...section,
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto p-3 text-xs">
          {filteredSections.map((section, sectionIndex) => {
            ...
            if (!section.isCollapsible) {
              return (
                <div key={section.category} className="mb-3 space-y-1">
                  {visibleItems.map((item) => {
                    const Icon = getNavIcon(item.href);
                    const active = isActiveRoute(pathname, item.href, activeHref);
                    return (
                      <Link
            ...
            return (
              <div
                ...
                className={sectionIndex > 0 ? 'mt-4 border-t border-slate-100 pt-3' : ''}
              >
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="inline-flex shrink-0 items-center px-1.5 py-0.5 rounded-md text-[9px] font-bold bg-slate-100 text-slate-500">
                      {visibleItems.length}
                    </span>
                  </div>
                  {isCollapsed ? (
                ...
                {!isCollapsed && (
                  <div className="mt-1 ml-1 space-y-0.5 border-l-2 border-slate-100 pl-2">
                    {visibleItems.map((item) => {
                      const Icon = getNavIcon(item.href);
                      const active = isActiveRoute(pathname, item.href, activeHref);
                      return (
