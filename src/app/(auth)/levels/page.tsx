...
export default function LevelsMasterPage() {
  ...
  return (
    <div className="space-y-6 p-6">
      <PageHeaderCard
        title="Levels Master"
        description="Configure vertical rack levels & shelf tiers (L01, L02, L03, L04...)"
        badge="Warehouse Structure · Levels"
        icon={Layers}
      >
        <button onClick={handleRefresh} disabled={isFetching} className="p-2 text-slate-200 hover:text-white hover:bg-white/10 rounded-xl transition" title="Refresh">
          <RefreshCw className={`h-4 w-4 ${isFetching ? 'animate-spin text-teal-300' : ''}`} />
        </button>
        <button onClick={() => setIsOpen(true)} className="px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 shadow-md transition">
