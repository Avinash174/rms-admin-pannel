...
export const columns: ColumnDef<Branch>[] = [
  {
    accessorKey: 'name',
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Branch Name</span>,
    cell: ({ row, table }) => {
      ...
      return (
        <div 
          ...
          onClick={() => meta?.onCustomAction?.(branch)}
        >
          <div
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const branch = row.original;
      const details = [branch.city, branch.state].filter(Boolean).join(', ');
      return (
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-xs font-mono text-slate-550 font-semibold">
        {row.getValue('phone') || '-'}
      </div>
    ),
  },
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <button
          onClick={(e) => {
            e.stopPropagation();
            meta?.onEdit?.({ ...branch, isActive: !isActive }, true);
          }}
  ...
  },
  {
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <ActionDropdown
          onEdit={() => meta?.onEdit?.(branch)}
          onDelete={() => meta?.onDelete?.(branch)}
