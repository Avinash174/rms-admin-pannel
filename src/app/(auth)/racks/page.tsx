...
import { CreateRackData, createRackSchema } from '@/lib/validations/rack';
...
export default function RacksPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => updateRack(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['racks'] });
      setIsFormDrawerOpen(false);
  ...
  const handleFormSubmit = (data: CreateRackData) => {
    if (formMode === 'CREATE') {
      createMutation.mutate(data);
    } else if (selectedRack) {
      updateMutation.mutate({ id: selectedRack.id, data });
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-[500px] space-y-4">
        ...
        <span className="text-sm font-semibold text-slate-500 animate-pulse">Loading racks...</span>
      </div>
    );
  }
  ...
  const activeCount = racks.filter(r => r.isActive).length;
  ...
  return (
    <div className="w-full space-y-6 px-4 sm:px-6 lg:px-0 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Rack Management</h1>
          <p className="text-sm text-slate-500 mt-1">Manage physical storage rack structures within rooms.</p>
        </div>
        <Button 
          onClick={() => {
            setFormMode('CREATE');
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-150 shadow-sm hover:shadow-md transition-all duration-300 group">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div className="space-y-2">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Racks</p>
          ...
          <div className="mt-5 text-xs text-slate-400 flex items-center gap-1.5 border-t border-slate-50 pt-4">
            <Info className="w-4 h-4 text-blue-500" /> Physical storage racks registered
          </div>
        </div>
        ...
        <div className="relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-150 shadow-sm hover:shadow-md transition-all duration-300 group">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div className="space-y-2">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Active Racks</p>
        ...
        <div className="relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-150 shadow-sm hover:shadow-md transition-all duration-300 group">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div className="space-y-2">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Inactive Racks</p>
          ...
          <div className="mt-5 text-xs text-slate-400 flex items-center gap-1.5 border-t border-slate-50 pt-4">
            <Info className="w-4 h-4 text-rose-500" /> Offline rack storage spaces
          </div>
        </div>
      </div>
        ...
        {filteredRacks.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-80 text-slate-400 p-6 space-y-3">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-slate-400">Add a new rack to start organizing physical containers</p>
            </div>
          </div>
        ) : (
          <DataTable columns={columns} data={filteredRacks}
            ...
            onEdit={(rack, isToggle) => {
              if (isToggle) {
                updateMutation.mutate({ id: rack.id, data: rack });
              } else {
      ...
      {/* SLIDE-OVER DRAWER: Add/Edit Rack */}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="name" placeholder="Aisle 1 Rack B" className="h-11 rounded-xl border-slate-200" {...form.register('name')} />
                    {form.formState.errors.name && <p className="text-xs text-red-500">{form.formState.errors.name.message}</p>}
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="code">Rack Code</Label>
                    <Input id="code" placeholder="A1RB" className="uppercase h-11 rounded-xl border-slate-200" {...form.register('code')} />
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[10px] text-slate-400 font-semibold">Enable or disable operator visibility for this rack</p>
                  </div>
                  <Switch
                    id="isActive"
      ...
      {/* SLIDE-OVER DRAWER: Rack Details */}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="flex flex-col items-center text-center p-6 bg-gradient-to-b from-blue-50/30 to-indigo-50/10 rounded-2xl border border-slate-100">
                  <div className="w-16 h-16 rounded-2xl bg-blue-600 text-white flex items-center justify-center font-bold text-lg shadow-md mb-3">
                    {selectedRackForDetail.code}
                  </div>
                  <h4 className="text-base font-extrabold text-slate-900">{selectedRackForDetail.name}</h4>
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-bold border mt-3 ${
                    selectedRackForDetail.isActive ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'
                  }`}>
                    {selectedRackForDetail.isActive ? 'Active' : 'Inactive'}
                  </span>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="divide-y divide-slate-100 border border-slate-100 rounded-2xl overflow-hidden bg-white shadow-xs">
                    <div className="flex justify-between items-center px-4 py-3">
                      ...
                      <span className="text-xs font-semibold text-slate-700">{selectedRackForDetail.roomName || '-'}</span>
                    </div>
                    <div className="flex justify-between px-4 py-3">
                      <span className="text-xs font-semibold text-slate-500">Description</span>
                      <span className="text-xs font-semibold text-slate-700 text-right">{selectedRackForDetail.description || '-'}</span>
                    </div>
                  </div>
                </div>
