...
import { MoreHorizontal, Pencil, Trash2, Calendar, FileBox } from 'lucide-react';
...
export const columns: ColumnDef<Rack>[] = [
  {
    accessorKey: 'name',
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Rack Name</span>,
    cell: ({ row, table }) => {
      const rack = row.original;
      const name = rack.name || 'Unknown Rack';
      ...
      return (
        <div 
          ...
          onClick={() => meta?.onCustomAction?.(rack)}
        >
          <div
  ...
  },
  {
    ...
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Room</span>,
    cell: ({ row }) => (
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <button
          onClick={(e) => {
            e.stopPropagation();
            meta?.onEdit?.({ ...rack, isActive: !isActive }, true);
          }}
          className={`group flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all duration-300 border ${
            isActive
              ? 'bg-emerald-50/60 text-emerald-750 text-emerald-700 border-emerald-250 border-emerald-200 hover:bg-emerald-100/80 shadow-sm shadow-emerald-500/5'
