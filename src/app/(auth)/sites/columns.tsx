...
export const columns: ColumnDef<Site>[] = [
  {
    accessorKey: 'name',
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Site Name</span>,
    cell: ({ row, table }) => {
      ...
      return (
        <div 
          ...
          onClick={() => meta?.onCustomAction?.(site)}
        >
          <div
          ...
          >
            {initials || <Map className="w-5 h-5" />}
          </div>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const site = row.original;
      const details = [site.city, site.state].filter(Boolean).join(', ');
      return (
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <button
          onClick={(e) => {
            e.stopPropagation();
            meta?.onEdit?.({ ...site, isActive: !isActive }, true);
          }}
