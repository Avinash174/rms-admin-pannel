...
export const columns: ColumnDef<Client>[] = [
  {
    accessorKey: 'name',
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Client Name</span>,
    cell: ({ row, table }) => {
      ...
      return (
        <div 
          ...
          onClick={() => meta?.onCustomAction?.(client)}
        >
          <div
  ...
  },
  {
    ...
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Contact Person</span>,
    cell: ({ row }) => (
  ...
  {
    accessorKey: 'email',
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Email</span>,
    cell: ({ row }) => (
      <div className="text-xs text-slate-550 font-mono font-semibold max-w-[150px] truncate">
        {row.getValue('email') || '-'}
      </div>
    ),
  },
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <button
          onClick={(e) => {
            e.stopPropagation();
            meta?.onEdit?.({ ...client, isActive: !isActive }, true);
          }}
  ...
  },
  {
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <ActionDropdown
          onEdit={() => meta?.onEdit?.(client)}
          onDelete={() => meta?.onDelete?.(client)}
