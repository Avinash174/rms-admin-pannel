...
export const columns: ColumnDef<Department>[] = [
  {
    accessorKey: 'name',
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Department Name</span>,
    cell: ({ row, table }) => {
      ...
      return (
        <div 
          ...
          onClick={() => meta?.onCustomAction?.(dept)}
        >
          <div
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <button
          onClick={(e) => {
            e.stopPropagation();
            meta?.onEdit?.({ ...dept, isActive: !isActive }, true);
          }}
  ...
  },
  {
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <ActionDropdown
          onEdit={() => meta?.onEdit?.(dept)}
          onDelete={() => meta?.onDelete?.(dept)}
