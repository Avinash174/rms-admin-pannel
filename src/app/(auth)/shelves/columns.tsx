...
export const columns: ColumnDef<Shelf>[] = [
  {
    accessorKey: 'name',
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Shelf Name</span>,
    cell: ({ row, table }) => {
      const shelf = row.original;
      const name = shelf.name || 'Unknown Shelf';
      ...
      return (
        <div 
          ...
          onClick={() => meta?.onCustomAction?.(shelf)}
        >
          <div
  ...
  },
  {
    ...
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Rack</span>,
    cell: ({ row }) => (
      <div className="text-xs text-slate-800 font-semibold">
        {row.getValue('rackName') || '-'}
      </div>
    ),
  },
  {
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <button
          onClick={(e) => {
            e.stopPropagation();
            meta?.onEdit?.({ ...shelf, isActive: !isActive }, true);
          }}
  ...
  },
  {
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <ActionDropdown
          onEdit={() => meta?.onEdit?.(shelf)}
          onDelete={() => meta?.onDelete?.(shelf)}
          deleteLabel="Delete Shelf"
        />
      );
    },
  },
