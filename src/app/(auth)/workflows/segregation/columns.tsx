...
import { Clock, Package, CheckCircle2, XCircle, User, ArrowRight, Tag } from 'lucide-react';
...
export const columns: ColumnDef<Segregation>[] = [
  {
    accessorKey: 'segregationCode',
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Segregation Code</span>,
    cell: ({ row, table }) => {
  ...
  },
  {
    ...
    header: () => <span className="text-xs font-bold tracking-wider text-slate-500 uppercase">Location Route</span>,
    cell: ({ row }) => {
      const item = row.original;
      return (
    ...
    cell: ({ row }) => (
      <div className="flex flex-col gap-1">
        ...
        {row.original.reasonCode && (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold font-mono text-violet-600 bg-violet-50 border border-violet-100 px-1.5 py-0.5 rounded w-fit">
            <Tag className="w-2.5 h-2.5" />{row.original.reasonCode}
          </span>
        )}
      </div>
    ),
