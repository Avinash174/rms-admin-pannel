...
import {
  ...
  Clock, Search, Sparkles, Info, X, User, Calendar, ArrowRight, Tag
} from 'lucide-react';
...
export default function SegregationPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<Segregation | null>(null);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[500px] space-y-4">
        <div className="relative flex items-center justify-center">
          ...
          <Package className="w-5 h-5 text-rose-600 absolute animate-pulse" />
        </div>
        <span className="text-sm font-semibold text-slate-500 animate-pulse">Loading segregation workflow...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-[500px] space-y-4">
        ...
        <h3 className="text-lg font-bold text-slate-900">Failed to load segregation operations</h3>
    ...
    <div className="w-full space-y-8 px-4 sm:px-6 lg:px-8 pb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-150 shadow-sm hover:shadow-md transition-all duration-300 group">
          <div className="absolute top-0 right-0 w-28 h-28 bg-gradient-to-bl from-rose-50 to-pink-50/30 rounded-bl-full opacity-80 group-hover:scale-105 transition-transform duration-500" />
          ...
          <div className="mt-5 text-xs text-slate-400 flex items-center gap-1.5 border-t border-slate-50 pt-4">
            <Info className="w-4 h-4 text-rose-500" /> Box segregation history
          </div>
        </div>
        ...
        <div className="relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-150 shadow-sm hover:shadow-md transition-all duration-300 group">
          ...
          <div className="mt-5 text-xs text-slate-400 flex items-center gap-1.5 border-t border-slate-50 pt-4">
            ...
            Segregation confirmed and closed
          </div>
        </div>
        ...
        <div className="relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-150 shadow-sm hover:shadow-md transition-all duration-300 group">
          ...
          <div className="mt-5 text-xs text-slate-400 flex items-center gap-1.5 border-t border-slate-50 pt-4">
            <Info className="w-4 h-4 text-amber-500" /> Awaiting physical relocation
          </div>
        </div>
      </div>
          ...
          <div className={`w-screen max-w-md bg-white shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${isDetailsOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-slate-900">Segregation Details</h3>
              </div>
            ...
            {selectedItem && (
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                <div className="flex flex-col items-center text-center p-6 bg-gradient-to-b from-rose-50/30 to-pink-50/10 rounded-2xl border border-slate-100">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-md mb-3 text-white ${
                    ...
                    selectedItem.status === 'IN_PROGRESS' ? 'bg-rose-600' :
                    selectedItem.status === 'FAILED' ? 'bg-gray-600' : 'bg-amber-500'
                  }`}>
                    {selectedItem.status === 'COMPLETED' ? <CheckCircle className="w-6 h-6" /> : <Package className="w-6 h-6" />}
                  </div>
                  <h4 className="text-base font-extrabold text-slate-900">{selectedItem.segregationCode}</h4>
                  <p className="text-xs text-slate-500 mt-1">{selectedItem.boxName || selectedItem.boxBarcode}</p>
                  {selectedItem.reasonCode && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-bold font-mono text-violet-600 bg-violet-50 border border-violet-100 px-2 py-0.5 rounded mt-1">
                      <Tag className="w-2.5 h-2.5" />{selectedItem.reasonCode}
                    </span>
                  )}
                ...
                <div className="space-y-3">
                  <h5 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Location Route</h5>
